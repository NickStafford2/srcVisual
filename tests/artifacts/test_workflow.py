...
from srcvisual.workflow.payload import build_artifact_backed_visualization

EXAMPLES_DIR = Path(__file__).resolve().parents[2] / "examples"


def test_legacy_payload_is_projected_from_published_canonical_artifact(
    tmp_path: Path,
) -> None:
    _example = EXAMPLES_DIR / "e2e_generated_to_new_file_diff.xml"
    _input = _example.read_bytes()

    _published, _legacy_payload = build_artifact_backed_visualization(
    ...
    assert "<diff:ws" not in _legacy_payload.moved_srcdiff_xml
    ...
    assert _legacy_payload.move_results["moves"][0]["match_kind"] == "exact"
...
def test_single_root_input_round_trips_through_artifact(tmp_path: Path) -> None:
    ...
    assert _stored.payload.files[0].revision_file.filename == (
        _legacy_payload.files[0].revision_file.filename
    )
    assert _stored.payload.files[0].tree is not None
    assert _legacy_payload.files[0].tree is not None
