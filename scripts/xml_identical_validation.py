...
NamespaceBinding = tuple[str, str]


def iter_namespace_bindings(xml_path: Path) -> Iterator[NamespaceBinding]:
    for _event, data in ET.iterparse(xml_path, events=("start-ns",)):
        assert isinstance(data, tuple), (
            f"Expected namespace binding tuple while parsing {xml_path}, "
            f"got {type(data).__name__}."
        )
        assert len(data) == 2, (
            f"Expected namespace binding tuple of length 2 while parsing {xml_path}, "
            f"got {len(data)}."
        )
        ...
        assert isinstance(prefix, str), (
            f"Expected namespace prefix to be a string while parsing {xml_path}, "
            f"got {type(prefix).__name__}."
        )
        assert isinstance(uri, str), (
            f"Expected namespace URI to be a string while parsing {xml_path}, "
...
def collect_ignored_namespace_uris(xml_path: Path) -> set[str]:
    ...
    for prefix, uri in iter_namespace_bindings(xml_path):
...
def format_namespace_bindings(bindings: dict[str, set[str]]) -> dict[str, list[str]]:
    return {uri: sorted(prefixes) for uri, prefixes in sorted(bindings.items())}
